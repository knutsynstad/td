...
export const buildCastleFlowField = (
  opts: CorridorFlowFieldOptions
): CorridorFlowField => {
  ...
  for (const collider of opts.colliders) {
    const inflation = collider.type === 'castle' ? 2 * res : 0;
    ...
    const sx = Math.max(
      0,
      Math.min(width - 1, Math.ceil((minX - minWX) / res - eps))
    );
    const sz = Math.max(
      0,
      Math.min(height - 1, Math.ceil((minZ - minWZ) / res - eps))
    );
    const ex = Math.max(
      0,
      Math.min(width - 1, Math.floor((maxX - minWX) / res - eps))
    );
    const ez = Math.max(
      0,
      Math.min(height - 1, Math.floor((maxZ - minWZ) / res - eps))
    );
  ...
  for (let d = 1; d <= maxDistance; d += 1) {
    ...
    for (const idx of layer) {
      ...
      for (let dir = 0; dir < DIRECTION_TO_OFFSET.length; dir += 1) {
        ...
        if (
          turns < bestTurns ||
          (turns === bestTurns && nIdx === nextToGoal[idx])
        ) {
          bestTurns = turns;
          bestNeighbor = nIdx;
          bestDir = dir;
...
export const tracePathFromSpawner = (
  field: CorridorFlowField,
  opts: TraceOptions
): CorridorPathResult => {
  const toCellNearest = (x: number, z: number): [number, number] => {
    const cx = Math.max(
      ...
      Math.min(
        field.width - 1,
        Math.round((x - field.minWX) / field.resolution)
      )
    );
    const cz = Math.max(
      0,
      Math.min(
        field.height - 1,
        Math.round((z - field.minWZ) / field.resolution)
      )
    );
    return [cx, cz];
  };
  ...
  const toWorld = (x: number, z: number) =>
    new THREE.Vector3(
      field.minWX + x * field.resolution,
      0,
      field.minWZ + z * field.resolution
    );
  ...
  const hasCardinalCorridorClearance = (
    from: THREE.Vector3,
    to: THREE.Vector3
  ) => {
  ...
  if (field.goals.length > 0) {
    ...
    const oneCellCenterSnap =
      Math.abs(goalCellX - lastCellX) + Math.abs(goalCellZ - lastCellZ) <= 1;
    if (
      last.distanceToSquared(bestGoal) > 1e-6 &&
      (hasCardinalCorridorClearance(last, bestGoal) || oneCellCenterSnap)
    ) {
