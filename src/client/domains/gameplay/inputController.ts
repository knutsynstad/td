...
export const createInputController = () => {
  ...
  return {
    ...
    getKeyboardMoveDirection(
      context: KeyboardDirectionContext
    ): THREE.Vector3 | null {
      const up =
        pressedMovementKeys.has('KeyW') || pressedMovementKeys.has('ArrowUp')
          ? 1
          : 0;
      const down =
        pressedMovementKeys.has('KeyS') || pressedMovementKeys.has('ArrowDown')
          ? 1
          : 0;
      const left =
        pressedMovementKeys.has('KeyA') || pressedMovementKeys.has('ArrowLeft')
          ? 1
          : 0;
      const right =
        pressedMovementKeys.has('KeyD') || pressedMovementKeys.has('ArrowRight')
          ? 1
          : 0;
      ...
      context.keyboardRight.set(
        -context.keyboardForward.z,
        0,
        context.keyboardForward.x
      );
