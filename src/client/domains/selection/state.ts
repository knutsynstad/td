...
export const isColliderInRange = (
  ...
  const dx =
    Math.abs(player.mesh.position.x - collider.center.x) - collider.halfSize.x;
  const dz =
    Math.abs(player.mesh.position.z - collider.center.z) - collider.halfSize.z;
...
export const getSelectedInRange = (
  ...
  Array.from(selection.selectedStructures).filter((collider) =>
    isColliderInRange(player, collider, range)
  );

export const getSingleSelectedTower = (
  selection: SelectionState,
  structureStore: StructureStore
): Tower | null => {
