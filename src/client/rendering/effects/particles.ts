...
export const createParticleSystem = (scene: THREE.Scene): ParticleSystem => {
  ...
  const createCubeParticle = (
    ...
    const maxLifetime =
      lifetimeMin + Math.random() * (lifetimeMax - lifetimeMin);
    const angularScale = options.angularVelocityScale ?? 10;
    const mesh = new THREE.Mesh(
      new THREE.BoxGeometry(size, size, size),
      new THREE.MeshStandardMaterial({
        color: options.color ?? 0xff7a7a,
        transparent: true,
      })
    );
  ...
  const spawnCubeEffects = (
    pos: THREE.Vector3,
    options: CubeEffectOptions = {}
  ) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      particles.push(
        createCubeParticle(pos.clone(), velocity, options.particle)
      );
    }
  };
  ...
  const spawnCoinRewardEffects = (pos: THREE.Vector3) => {
    if (coinParticleTemplate) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        particles.push(
          createModelParticle(pos.clone(), velocity, coinParticleTemplate)
        );
      }
      return;
    }
