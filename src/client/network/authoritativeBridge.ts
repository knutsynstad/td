...
type AuthoritativeBridge = {
  playerId: string;
  sendMoveIntent: (position: Vec2, target: Vec2, desiredDir?: Vec2) => Promise<void>;
...
const isJoinResponse = (value: unknown): value is JoinResponse => {
  ...
  if (typeof value.playerId !== "string" || typeof value.username !== "string" || typeof value.channel !== "string") {
    return false;
  }
  if (!isRecord(value.snapshot) || !isRecord(value.snapshot.players)) return false;
  return true;
};
...
const applyDelta = (delta: GameDelta, callbacks: PresenceCallbacks): void => {
  if (delta.type === "presenceDelta") {
    if (delta.joined) {
      callbacks.onRemoteJoin(delta.joined.playerId, delta.joined.username, delta.joined.position);
    }
    if (delta.left) {
...
export const connectAuthoritativeBridge = async (callbacks: PresenceCallbacks): Promise<AuthoritativeBridge> => {
  ...
  const sendCommand = async (command: CommandRequest["envelope"]["command"]): Promise<void> => {
    seq += 1;
