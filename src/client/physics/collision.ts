...
import type { Entity, StaticCollider } from '../game/types'
...
export const resolveCircleAabb = (pos: THREE.Vector3, radius: number, collider: StaticCollider) => {
...
export const distanceToColliderSurface = (pos: THREE.Vector3, radius: number, collider: StaticCollider): number => {
  const minX = collider.center.x - collider.halfSize.x
  const maxX = collider.center.x + collider.halfSize.x
  const minZ = collider.center.z - collider.halfSize.z
  const maxZ = collider.center.z + collider.halfSize.z
