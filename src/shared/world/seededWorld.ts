  ...
  apply: (context, state) => {
    ...
    for (let x = -maxCoord; x <= maxCoord; x += context.gridStep) {
      for (let z = -maxCoord; z <= maxCoord; z += context.gridStep) {
        ...
        const clumpBoost =
          Math.max(0, macro - 0.56) * 1.25 + Math.max(0, local - 0.62) * 0.9;
        const radialDistance = Math.hypot(x, z);
        const gradientRange = Math.max(
          1,
          gradientEndDistance - gradientStartDistance
        );
        const gradientT = Math.max(
          0,
          Math.min(1, (radialDistance - gradientStartDistance) / gradientRange)
        );
        const edgeDensityBoost = Math.pow(smoothStep(gradientT), 0.45);
        const density = Math.min(
          0.3,
          baseDensity + clumpBoost * (0.01 + edgeDensityBoost * 0.28)
        );
...
export const createDefaultWorldGenRules = (): WorldGenRule[] => [
  treeRule,
  rockRule,
];

export const generateSeededWorldFeatures = (
  config: SeededWorldGenConfig
): GeneratedWorldFeatures => {
