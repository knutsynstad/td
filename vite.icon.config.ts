...
const saveIconPlugin = () => ({
  ...
  configureServer(server: { middlewares: { use: (fn: (req: unknown, res: unknown, next: () => void) => void) => void } }) {
    server.middlewares.use(async (req: { url?: string; method?: string; on: (e: string, fn: (chunk: Buffer) => void) => void }, res: { setHeader: (a: string, b: string) => void; statusCode: number; end: (s?: string) => void }, next: () => void) => {
      if (req.url !== '/__save-icon' || req.method !== 'POST') return next();
      const chunks: Buffer[] = [];
      req.on('data', (chunk: Buffer) => chunks.push(chunk));
      req.on('end', async () => {
        try {
          const body = JSON.parse(Buffer.concat(chunks).toString()) as { filename: string; dataUrl: string };
          ...
          await writeFile(join(dir, body.filename), Buffer.from(match[1], 'base64'));
          res.setHeader('Content-Type', 'application/json');
          res.statusCode = 200;
